...
fn main() {
  ...
  let device = WgpuDevice::default(); // Default WGPU device

  println!("YOLOv3 Burn Example");
  println!(
    "Using backend: {:?}",
    std::any::type_name::<MyAutodiffBackend>()
  );
  ...
  let model: YoloV3<MyAutodiffBackend> = model_config.init(&device);

  println!(
    "Model initialized with {} classes and {} anchors per scale.",
  ...
  println!(
    "Dummy input tensor created with shape: [1, 3, {}, {}]",
  ...
  println!(
    "Applying Non-Maximum Suppression with IoU threshold: {} and Confidence threshold: {}",
